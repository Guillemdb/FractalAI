from .environment import Environment, OpenAIEnvironment, AtariEnvironment, DMControlEnv
...
def run_demo():
    environment = AtariEnvironment(name="MsPacman-v0", clone_seeds=True)
    model = RandomDiscreteModel(environment.num_actions)

    agent = FractalAI(
        policy=GreedyPolicy(env=environment, model=model),
        max_samples=300,
        max_states=15,
        time_horizon=10,
        n_fixed_steps=5,
    ...
    AtariMonitorPolicy(policy=agent, directory="videos", force=True).run(skip_frames=80)
